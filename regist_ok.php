...
// Check the value of usertype
if ($usertype == '') {
    echo "<script>alert('회원 유형을 선택해주세요.'); history.back();</script>";
    exit;
}

// Prepare the SQL query using a prepared statement
$sql = "INSERT INTO user_info(user_id, user_pass, user_email, user_type) VALUES (?, ?, ?, ?)";
$stmt = mysqli_stmt_init($conn);
$prepareStmt = mysqli_stmt_prepare($stmt, $sql);

if ($prepareStmt) {
    mysqli_stmt_bind_param($stmt, "ssss", $userid, $userpass, $useremail, $usertype);
